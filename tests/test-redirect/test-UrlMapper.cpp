...
TEST(TestUrlMapper, maps_matching_path_prefix)
{
    ...
    boost::urls::url url =
        mapper.add_rule("/api", "https://api.internal/v1")("https://proxy.example.com/api/users?id=42#row");
...
TEST(TestUrlMapper, uses_longest_matching_path_prefix)
{
    ...
    boost::urls::url url =
        mapper.add_rule("/api", "https://api.internal")
            .add_rule("/api/admin", "https://admin.internal/root")("https://proxy.example.com/api/admin/users");
...
TEST(TestUrlMapper, preserves_encoded_query)
{
    ...
    boost::urls::url url =
        mapper.add_rule("/search", "https://search.internal")("https://proxy.example.com/search?q=boost%20url&page=2");
...
TEST(TestUrlMapper, throws_when_no_rule_matches)
{
    ...
    ASSERT_THROW(
        mapper.add_rule("/api", "https://api.internal")("https://proxy.example.com/other"), std::runtime_error);
}
